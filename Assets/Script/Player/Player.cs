...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (gameManager == null) {
            Debug.LogWarning("GameManager not found in the scene. Some features may be disabled.");
        }
    }
    ...
    void FixedUpdate()
    {
        if (isInWater)
        {
        Vector2 waterMove = GetComponent<PlayerInput>().actions["Move"].ReadValue<Vector2>();
        rb.velocity = waterMove * speed;
        }
        if (!isDashing)
        {
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("Item")) {
            ...
            if (gameManager != null) {
                ...
                Debug.LogWarning("Item consumed, but no GameManager to process it.");
            }
        }
